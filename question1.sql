    ...
    ROUND(
        SUM(
            CASE 
                ...
                ELSE 0
            END
        ) * 1.0 / SUM(v.total_population), 4
    ) AS beneficiaries_to_population_ratio
FROM beneficiary_partner_data b
JOIN village_locations v ON b.village = v.village
JOIN jurisdiction_hierarchy jv ON jv.name = v.village
JOIN jurisdiction_hierarchy d ON jv.parent = d.name
...
SELECT
    ...
    COUNT(DISTINCT d.name) AS districts_reached
FROM beneficiary_partner_data b
JOIN jurisdiction_hierarchy jv ON b.village = jv.name
JOIN jurisdiction_hierarchy d ON jv.parent = d.name
GROUP BY b.partner;
